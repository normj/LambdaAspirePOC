...
namespace Aspire.Hosting.LambdaEmulator;
...
public class RuntimeApiController : ControllerBase
{
    ...
    public async Task GetNextInvocation(CancellationToken token = default)
    {
        IEventContainer? activeEvent;
        while (!_runtimeApiDataStore.TryActivateEvent(out activeEvent) && !token.IsCancellationRequested)
        {
